...
import Test.QuickCheck.Gen (elements, listOf1, oneof)

data URIAuth = URIAuth
  { uriAuthUser :: !(Maybe Text)
  -- ^ a designated user - @ssh://git\@github.com@ is @git@
  , uriAuthPassword :: !(Maybe Text)
  -- ^ a designated password (this field is depricated in RFC 3986, passwords with an at-character will not parse) - @https://user:password\@github.com@ is @password@
  , uriAuthHost :: !URIAuthHost
  , uriAuthPort :: !(Maybe Word16)
  -- ^ the port, if it exists - @foobar.com:3000@ is @3000@ as a 16-bit unsigned int.
  }
